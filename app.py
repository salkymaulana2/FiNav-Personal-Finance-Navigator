...
st.set_page_config(
    ...
    page_icon="💸", # Keep browser tab favicon simple
    layout="wide"
)

# Custom CSS for an ultra-clean, icon-free premium look
st.markdown("""
    <style>
    ...
    /* Action Controls: Emerald FinTech Green */
...
if GEMINI_API_KEY == "YOUR_GEMINI_API_KEY_HERE":
    st.error("API Key Missing: Please replace the placeholder string on line 91 with your key.")
    st.stop()
...
with st.sidebar:
    ...
    <div style='color: #FFFFFF !important; opacity: 1.0 !important; font-size: 0.85rem; line-height: 1.6;'>
        <b>Deployment Context:</b> Python / Streamlit Local<br><br>
...
# Using Streamlit's native modern icons system instead of emoji clutter
    ...
    with col_table:
        ...
        if not st.session_state.expense_log.empty:
            for index, row in st.session_state.expense_log.iterrows():
                ...
                # Replaced raw trash emoji with a clean word-link button style
...
with tab_investment:
    ...
    with col_inv_outputs:
        ...
        st.metric("Modeled Future Horizon Wealth Balance", f"${final_extrapolated_wealth:,.2f}", delta=f"Based on recurring ${annual_contribution_base:,.2f}/yr deposits")
        ...
        ax_line.set_xlabel("Timeline Horizon Vector (Years)")
        ax_line.set_ylabel("Portfolio Capital Valuation Base ($)")
        ax_line.set_title("Extrapolated Compound Capital Growth Curve")
        ...
        if st.button("Generate Asset Allocation Map"):
            with st.spinner("Analyzing market trajectories..."):
                ...
                Detail a rigorous macroeconomic asset-class distribution layout model (e.g., precise percentage allocations across Equities, Fixed Income Bonds, sovereign Treasuries) tailored precisely for a portfolio structured inside a {investment_risk} strategy profile. Do not use emojis in your response.
                """
